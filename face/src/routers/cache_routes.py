...
from connection import create_vector_index, add_face_to_index, identify_face_imread_with_vector_search, dump_index_face, find_face_internal, add_face_to_index_base64, identify_multipel_face_imread_with_vector_search
...
@router.post("/face/findIDs/base64")
async def find_face_by_base64_multiple(item: FaceRequest):
  base64_str = item.base64
  if not base64_str:
      return {
          "status": False,
  ...
  image_path = DecodingImage(base64_str)
  result = identify_multipel_face_imread_with_vector_search(image_path)
  if os.path.exists(image_path):
     os.remove(image_path)
  return {
          "status": True,
