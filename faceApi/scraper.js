...
/**
 * Aplikasi Scraper SatuSehat SDMK
 * Mengambil data tenaga kesehatan dan menyimpan foto dengan noSTR
 * 
 * Penggunaan:
 * node scraper.js <URL atau noSTR>
 * 
 * Contoh:
 ...
 */
async function scrapeSatuSehat(url) {
    console.log(`\n📋 Memulai scraping: ${url}\n`);
    ...
    try {
        ...
        console.log(`   Kompetensi: ${nakesData.kompetensi}`);
            // Extract SIP data - try multiple strategies:
            // 1) Desktop row blocks (flex rows with border-b)
            // 2) Mobile card blocks
        ...
        // let sipMatch;
        // const processedSIPs = new Map(); // Gunakan Map untuk track unique SIP + tempat
        
        // while ((sipMatch = sipPattern.exec(pageText)) !== null) {
        //     const key = `${sipMatch[1]}|${sipMatch[4]}`; // SIP + tempat praktik
            
        //     if (!processedSIPs.has(key)) {
        //         processedSIPs.set(key, {
        //             sipNo: sipMatch[1].trim(),
        //             tanggalTerbit: sipMatch[2].trim(),
        //             tanggalBerakhir: sipMatch[3].trim(),
        //             tempatPraktik: sipMatch[4].trim(),
        //             provinsi: sipMatch[5].trim(),
        //             kabKota: (sipMatch[6] + ' ' + sipMatch[7]).trim()
        //         });
        //     }
        // }
        
        // // Convert Map ke array
        // nakesData.sips = Array.from(processedSIPs.values());

        // console.log(`\n📜 SIP:  ${nakesData.sips.length} catatan ditemui`);

        // // Extract last updated - clean up regex untuk tidak ambil JS code
        // const lastUpdateMatch = pageText.match(/Sumber data SATUSEHAT SDMK terakhir diperbarui\s*:?\s*(\d+\s+\w+\s+\d{4}[\d\s:,WIB]*)/i);
        // if (lastUpdateMatch) {
        //     nakesData.lastUpdated = lastUpdateMatch[1].trim().split('$')[0].trim();
        // }
...
Scraping data tenaga kesehatan dari portal SatuSehat
...
Contoh:
  ...
  node scraper.js https://satusehat.kemkes.go.id/sdmk/nakes/IH00000634255514
    ...
    if (input.startsWith('http')) {
        ...
        // Assume it's a noSTR
        url = CONFIG.baseUrl + input;
    }

    const result = await scrapeSatuSehat(url);
